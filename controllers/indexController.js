  ...
  exports.Signup =  async (req, res, next) => {
    try {
      ...
      await passport.authenticate("local")(req,res,function(){
        res.redirect("/mainPage")   /* Make it a perfect redirect */ 
      });
    //   res.status(201).json({ 
    //     success: true,
  ...
  exports.Signin = async (req, res, next) => {
    try {
      passport.authenticate("local", {
        successRedirect: "/mainPage", // Make it a success redirect
        failureRedirect: "/",
      })(req, res, next);
    } catch (error) {
...
exports.Add_Student_Profile = async (req, res, next) => {
  ...
  exports.List_of_Interviews = async (req, res, next) => {
    try {
        const list = await interviewModel.find(); // Await the result of the find() method
        console.log(list)
...
exports.ScheduleD_Interview = async (req, res, next) => {
    try {
        ...
        // Associate the interview with the student
...
exports.updateResult  =  async (req, res, next) =>  {
    try {
      ...
      // Update the Results field in the database
      interview.Interviews_Details.Results = resultValue;
      await interview.save();
  
      // Redirect back to the list of interviews
